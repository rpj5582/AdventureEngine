...
namespace AdventureEngine
{
	...
	{
		if (m_modelIDs.find(filepath) == m_modelIDs.end())
		{
			...
			if (ifs.is_open())
			{
				...
				return bufferModel(filepath, positions, uvs, normals, indices);
			}
		}
	...
	{
		return loadTexture(filepath, glm::uvec2(1, 1));
	}

	Texture* AssetManager::loadTexture(std::string filepath, glm::uvec2 atlasSize)
	{
		if (m_textureIDs.find(filepath) == m_textureIDs.end())
		{
			...
			m_textureIDs[filepath] = textureID;
	...
	Model* AssetManager::getModel(std::string filepath)
	{
		if (m_modelPrimitives.find(filepath) != m_modelPrimitives.end())
		{
			return m_models.at(m_modelPrimitives.at(filepath));
		}

		if (m_modelIDs.find(filepath) != m_modelIDs.end())
		{
			return m_models.at(m_modelIDs.at(filepath));
		}
	...
	Texture* AssetManager::getTexture(std::string filepath)
	{
		if (m_textureIDs.find(filepath) != m_textureIDs.end())
		{
			return m_textures.at(m_textureIDs.at(filepath));
		}
	...
	Model* AssetManager::bufferModel(std::string filepath, const std::vector<GLfloat> positionData, const std::vector<GLfloat> uvData, const std::vector<GLfloat> normalData, const std::vector<GLuint> indices)
	{
