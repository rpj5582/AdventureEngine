...
namespace AdventureEngine
{
	...
	bool Shader::load()
	{
		...
		GLuint vertexShaderID = load(m_vertexFile, GL_VERTEX_SHADER);
		if (vertexShaderID == 0) return false;

		GLuint fragmentShaderID = load(m_fragmentFile, GL_FRAGMENT_SHADER);
	...
	GLuint Shader::load(const char* filepath, GLenum shaderType)
	{
